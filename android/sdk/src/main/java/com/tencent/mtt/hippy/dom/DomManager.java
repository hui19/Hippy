...
public class DomManager implements HippyInstanceLifecycleEventListener,
    ...
    private static boolean jsJustLayout(HippyMap props) {
        ...
        if (props.get(NodeProps.COLLAPSABLE) != null
                && (Boolean) props.get(NodeProps.COLLAPSABLE) == false) {
            return false;
        }
    ...
    public void createNode(final HippyRootView hippyRootView, final int id, int pid, int index,
            final String className, HippyMap map) {
        //	assertThread();
        ...
        if (parentNode != null) {
            ...
            if (!isLayoutOnly && !node.isVirtual()) {
                ...
                addUITask(new IDomExecutor() {
                    ...
                    public void exec() {
                        mRenderManager
                                .createNode(hippyRootView, id, nativeParentNode.getId(), childIndex.mIndex, className,
                                        newProps);
                    }
                });
            }
