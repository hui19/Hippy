...
public class NativeGestureDispatcher implements NativeGestureProcessor.Callback {
  ...
  public static void reportGestureEventCallStack(HippyEngineContext context, String funcName,
    String msg) {
    if (context != null) {
      HippyEngineMonitorAdapter monitorAdapter = context.getGlobalConfigs()
        .getEngineMonitorAdapter();
...
//	}
  ...
  public void handle(String type, float x, float y) {
    ...
    if (TextUtils.equals(type, NodeProps.ON_PRESS_IN)) {
      reportGestureEventCallStack(mEngineContext, "handle",
        "ON_PRESS_IN id=" + mTargetView.getId());
