...
public class TextNode extends StyleNode {
  ...
  private StaticLayout createLayoutWithNumberOfLine(int lastLineStart, int lastLineEnd, int width) {
    ...
    String newString = text.subSequence(0, lastLineStart).toString()
      + truncate(text.substring(lastLineStart, lastLineEnd), sTextPaintInstance, width,
      mTruncateAt);
    ...
    StaticLayout staticLayout = new StaticLayout(temp.replace(start, text.length(), ELLIPSIS),
      sTextPaintInstance, width, mTextAlign, 1.f, 0.f,
      true);
