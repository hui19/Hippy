...
public class HippyBuffer {
    ...
    private static final byte[] BASE64_IMG_HEADER = new byte[]{'d', 'a', 't', 'a', ':', 'i', 'm', 'a',
            'g', 'e',
            '/'};
            ...
            switch (type) {
                ...
                default:
                    throw new RuntimeException(
                            "unknown hippy-buffer type " + type + " at " + mPosition + ", total buffer length ="
                                    + mBuffer.length);
            }
        }
...
//								+ "], percent:[" + (float) mCacheHitCount / mTotalCount * 100 + "]");
        ...
        private final long readLong() {
            long number = (((mBuffer[mPosition + 7] & 0xFFL)) + ((mBuffer[mPosition + 6] & 0xFFL) << 8)
                    + ((mBuffer[mPosition + 5] & 0xFFL) << 16)
                    + ((mBuffer[mPosition + 4] & 0xFFL) << 24) + ((mBuffer[mPosition + 3] & 0xFFL) << 32) + (
                    (mBuffer[mPosition + 2] & 0xFFL) << 40)
