...
public class AudioPlayerModule extends HippyNativeModuleBase {
  ...
  private void sendEvent(String eventName, HippyMap params) {
    this.mHippyEngineContext.getModuleManager().getJavaScriptModule(EventDispatcher.class)
      .receiveNativeEvent(eventName, params);
  }
  ...
  private MediaPlayerStateWrapper createMediaPlayer(Context context, Uri uri,
  ...
  public void play(String streamingURL, Promise callback) {
    try {
      ...
      this.mMediaPlayer = this
        .createMediaPlayer(this.mHippyEngineContext.getGlobalConfigs().getContext(), streamingURI,
