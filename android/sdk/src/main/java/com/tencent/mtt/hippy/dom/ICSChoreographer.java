...
public final class ICSChoreographer {
    ...
    private void postCallbackDelayedInternal(int callbackType, Object action, Object token,
            long delayMillis) {
        if (DEBUG) {
    ...
    private void removeCallbacksInternal(int callbackType, Object action, Object token) {
        if (DEBUG) {
            Log.d(TAG,
                    "RemoveCallbacks: type=" + callbackType + ", action=" + action + ", token=" + token);
        }
     ...
     * @see #removeFrameCallback
     */
    public void postFrameCallbackDelayed(HippyChoreographer.FrameCallback callback,
            long delayMillis) {
        if (callback == null) {
    ...
    void doFrame(long frameTimeNanos, int frame) {
        ...
        synchronized (mLock) {
            ...
            if (jitterNanos >= mFrameIntervalNanos) {
                ...
                if (DEBUG) {
                    Log.d(TAG,
                            "Missed vsync by " + (jitterNanos * 0.000001f) + " ms "
                                    + "which is more than the frame interval of "
                                    + (mFrameIntervalNanos * 0.000001f) + " ms!  " + "Skipping " + skippedFrames
                                    + " frames and setting frame "
        ...
        if (DEBUG) {
            ...
            Log.d(TAG, "Frame " + frame + ": Finished, took " + (endNanos - startNanos) * 0.000001f
                    + " ms, latency "
    ...
    void doCallbacks(int callbackType, long frameTimeNanos) {
        ...
        try {
            for (CallbackRecord c = callbacks; c != null; c = c.next) {
                if (DEBUG) {
                    Log.d(TAG,
                            "RunCallback: type=" + callbackType + ", action=" + c.action + ", token=" + c.token
                                    + ", latencyMillis="
    ...
    private final class CallbackQueue {
        ...
        public void removeCallbacksLocked(Object action, Object token) {
            ...
            for (CallbackRecord callback = mHead; callback != null; ) {
                ...
                if ((action == null || callback.action == action) && (token == null
                        || callback.token == token)) {
                    if (predecessor != null) {
