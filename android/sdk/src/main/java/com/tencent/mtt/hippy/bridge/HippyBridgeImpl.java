...
public class HippyBridgeImpl implements HippyBridge, DevRemoteDebugProxy.OnReceiveDataListener {
  ...
  public HippyBridgeImpl(HippyEngineContext engineContext, BridgeCallback callback,
    boolean singleThreadMode, boolean jsonBrige, boolean isDevModule) {
    ...
    synchronized (sBridgeSyncLock) {
      ...
      if (mCodeCacheRootDir == null) {
        ...
        if (hippyFile != null) {
          this.mCodeCacheRootDir =
            hippyFile.getAbsolutePath() + File.separator + "codecache" + File.separator;
        }
      }

      if (mCodeCacheThreadExecutor == null) {
        this.mCodeCacheThreadExecutor = new ThreadPoolExecutor(1, 1, 120L, TimeUnit.SECONDS,
  ...
  public void initJSBridge(String gobalConfig, NativeCallback callback, final int groupId) {
    ...
    if (this.mIsDevModule) {
      ...
      mDebugWebSocketClient.connect(
        String.format(Locale.US, "ws://%s/debugger-proxy?role=android_client", "localhost:38989"),
  ...
  private void initJSEngine(int groupId) {
    ...
    synchronized (HippyBridgeImpl.class) {
      mV8RuntimeId = initJSFramework(mDebugGobalConfig.getBytes(), mSingleThreadMode,
        mBridgeParamJson, mIsDevModule, mDebugInitJSFrameworkCallback, groupId);
      mInit = true;
    }
  }
  ...
  public boolean runScriptFromFile(String filePath, String scriptName, boolean canUseCodeCache,
    ...
    if (!TextUtils.isEmpty(codeCacheTag) && !TextUtils.isEmpty(mCodeCacheRootDir)) {
      LogUtils.e("HippyEngineMonitor",
        "runScriptFromFile ======core====== " + codeCacheTag + ", canUseCodeCache == "
          + canUseCodeCache);
      String codeCacheDir = mCodeCacheRootDir + codeCacheTag + File.separator;
      File file = new File(codeCacheDir);
      LogUtils.d("HippyEngineMonitor",
        "codeCacheDir file size : " + (file.listFiles() != null ? file.listFiles().length : 0));
      return runScriptFromFile(filePath, scriptName, canUseCodeCache, codeCacheDir, mV8RuntimeId,
        callback);
    } else {
      LogUtils.e("HippyEngineMonitor", "runScriptFromFile codeCacheTag is null");
      return runScriptFromFile(filePath, scriptName, false, "" + codeCacheTag + File.separator,
  ...
  public boolean runScriptFromAssets(String fileName, AssetManager assetManager,
    ...
    if (!TextUtils.isEmpty(codeCacheTag) && !TextUtils.isEmpty(mCodeCacheRootDir)) {
      LogUtils.e("HippyEngineMonitor",
        "runScriptFromAssets ======core====== " + codeCacheTag + ", canUseCodeCache == "
          + canUseCodeCache);
      String codeCacheDir = mCodeCacheRootDir + codeCacheTag + File.separator;
      File file = new File(codeCacheDir);
      LogUtils.d("HippyEngineMonitor",
        "codeCacheDir file size : " + (file.listFiles() != null ? file.listFiles().length : 0));
      return runScriptFromAssets(fileName, assetManager, canUseCodeCache, codeCacheDir,
        mV8RuntimeId, callback);
    } else {
      LogUtils.e("HippyEngineMonitor", "runScriptFromAssets codeCacheTag is null");
      return runScriptFromAssets(fileName, assetManager, false, "" + codeCacheTag + File.separator,
        mV8RuntimeId, callback);
    }
  }
  ...
  public boolean runScriptFromUri(String uri, AssetManager assetManager, boolean canUseCodeCache,
    ...
    if (!TextUtils.isEmpty(codeCacheTag) && !TextUtils.isEmpty(mCodeCacheRootDir)) {
      ...
      LogUtils.e("HippyEngineMonitor", "runScriptFromAssets codeCacheTag is null");
      return runScriptFromUri(uri, assetManager, false, "" + codeCacheTag + File.separator,
        mV8RuntimeId, callback);
    }
  }
  ...
  public void callFunction(String action, byte[] bytes, int offset, int length,
    ...
    if (!mInit || TextUtils.isEmpty(action) || bytes == null || bytes.length == 0 || offset < 0
  ...
  public native long initJSFramework(byte[] gobalConfig, boolean useLowMemoryMode,
    boolean useBrigeParamJson, boolean isDevModule, NativeCallback callback, long groupId);

  public native boolean runScriptFromFile(String filePath, String scriptName,
    boolean canUseCodeCache, String codeCacheDir, long V8RuntimId, NativeCallback callback);

  public native boolean runScriptFromAssets(String fileName, AssetManager assetManager,
    boolean canUseCodeCache, String codeCacheDir, long V8RuntimId, NativeCallback callback);
  ...
  public native void callFunction(String action, byte[] params, int offset, int length,
    long V8RuntimId, NativeCallback callback);

  public native void runNativeRunnable(String codeCacheFile, long nativeRunnableId, long V8RuntimId,
    NativeCallback callback);
